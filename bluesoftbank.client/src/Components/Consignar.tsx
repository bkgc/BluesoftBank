...
const ConsignarAccount: React.FC<{ id: string }> = ({ id }) => {
    ...
    const PostConsignacion = async () => {
        try {
            const response = await axios.post(`https://localhost:7222/api/Cuenta/consignar/${id}`, {
                saldo: saldo
            });
            console.log('Respuesta del servidor:', response.data);
            toast(response.data)
    ...
    return (<>
        <Button onClick={GetAccount}>
            <Typography>Consignar</Typography>
        </Button>
        <Dialog onClose={handleClose} open={state}>
            ...
            <Grid container spacing={2} style={{ width: "100%", padding: 20 }}>
                ...
                <Grid item xs={12}>
                    <Button onClick={(() => PostConsignacion())} style={{ backgroundColor: "#009aff", borderRadius: 10, marginLeft: 10 }}>
...
export default ConsignarAccount